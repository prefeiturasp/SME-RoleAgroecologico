...
if [ -n "${ENABLE_WORDPRESS_EXCLUSIONS}" ] && [ ${ENABLE_WORDPRESS_EXCLUSIONS} -eq 1 ]; then
  # Check if WordPress exclusions are already added
  if ! grep -q "WordPress exclusions" /etc/modsecurity.d/owasp-crs/crs-setup.conf; then
    cat <<EOF >> /etc/modsecurity.d/owasp-crs/crs-setup.conf

# WordPress exclusions
# Disable problematic rules for WordPress admin
SecAction \\
 "id:900130,\\
  phase:1,\\
  nolog,\\
